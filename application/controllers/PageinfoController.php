...
use App\Models\Backend-PageinfoModel;

class PageinfoController extends ResourceController {
  ...
  private function total_count_after_filter($sql=""){
	...
	public function index(){
      ...
      $logged_site_id = $this->request->getVar("logged_site_id");
			...
			$sql = "SELECT * FROM ( SELECT PI_SYS_ID, PI_ST_SYS_ID, PI_PAGE_DESC, PI_CODE, PI_TABLE_1, PI_TABLE_2, PI_TABLE_3, PI_ACTIVE_YN, PI_LANG_CODE, PI_CR_UID, PI_CR_DT, PI_UPD_UID, PI_UPD_DT FROM SITE_M_PAGE_INFO )  ";
			...
			if($result_array){
				...
				foreach ($result_array as $key => $value) {
					$name_array = [
						...
						"@user_end_parameter_PI_SYS_ID"	=>	$value["PI_SYS_ID"],
						"@user_end_parameter_PI_ST_SYS_ID"	=>	$value["PI_ST_SYS_ID"],
						"@user_end_parameter_PI_PAGE_DESC"	=>	$value["PI_PAGE_DESC"],
						"@user_end_parameter_PI_CODE"	=>	$value["PI_CODE"],
						"@user_end_parameter_PI_TABLE_1"	=>	$value["PI_TABLE_1"],
						"@user_end_parameter_PI_TABLE_2"	=>	$value["PI_TABLE_2"],
						"@user_end_parameter_PI_TABLE_3"	=>	$value["PI_TABLE_3"],
						"@user_end_parameter_PI_ACTIVE_YN"	=>	$value["PI_ACTIVE_YN"],
						"@user_end_parameter_PI_LANG_CODE"	=>	$value["PI_LANG_CODE"],
						"@user_end_parameter_PI_CR_UID"	=>	$value["PI_CR_UID"],
						"@user_end_parameter_PI_CR_DT"	=>	$value["PI_CR_DT"],
						"@user_end_parameter_PI_UPD_UID"	=>	$value["PI_UPD_UID"],
						"@user_end_parameter_PI_UPD_DT"	=>	$value["PI_UPD_DT"],
					];
					$data_to_send[] = $name_array;
				}
				$primary_key = "PRIMARY_KEY_NEED_TO_SEND";
        $table_name = "SITE_M_PAGE_INFO";
        $site_column = "";
	...
	public function create(){
		try{
			...
			$_P_PI_ST_SYS_ID = $this->request->getVar("@user_end_parameter_PI_ST_SYS_ID");
			$_P_PI_PAGE_DESC = $this->request->getVar("@user_end_parameter_PI_PAGE_DESC");
			$_P_PI_CODE = $this->request->getVar("@user_end_parameter_PI_CODE");
			$_P_PI_TABLE_1 = $this->request->getVar("@user_end_parameter_PI_TABLE_1");
			$_P_PI_TABLE_2 = $this->request->getVar("@user_end_parameter_PI_TABLE_2");
			$_P_PI_TABLE_3 = $this->request->getVar("@user_end_parameter_PI_TABLE_3");
			$_P_PI_ACTIVE_YN = $this->request->getVar("@user_end_parameter_PI_ACTIVE_YN");
			...
			$params = array(
				...
				array("name"=>":P_PI_ST_SYS_ID", "value"=>$_P_PI_ST_SYS_ID, "type"=>SQLT_CHR, "length"=>22),
				array("name"=>":P_PI_PAGE_DESC", "value"=>$_P_PI_PAGE_DESC, "type"=>SQLT_CHR, "length"=>250),
				array("name"=>":P_PI_CODE", "value"=>$_P_PI_CODE, "type"=>SQLT_CHR, "length"=>30),
				array("name"=>":P_PI_TABLE_1", "value"=>$_P_PI_TABLE_1, "type"=>SQLT_CHR, "length"=>50),
				array("name"=>":P_PI_TABLE_2", "value"=>$_P_PI_TABLE_2, "type"=>SQLT_CHR, "length"=>50),
				array("name"=>":P_PI_TABLE_3", "value"=>$_P_PI_TABLE_3, "type"=>SQLT_CHR, "length"=>50),
				array("name"=>":P_PI_ACTIVE_YN", "value"=>$_P_PI_ACTIVE_YN, "type"=>SQLT_CHR, "length"=>1),
			...
			$datatoShow = $this->db->storedProcedure("SPINE_SITE","INSERT_SITE_M_PAGE_INFO", $params);
	...
	public function new(){
		try{
			...
			$sql = " SELECT PI_SYS_ID, PI_ST_SYS_ID, PI_PAGE_DESC, PI_CODE, PI_TABLE_1, PI_TABLE_2, PI_TABLE_3, PI_ACTIVE_YN, PI_LANG_CODE, PI_CR_UID, PI_CR_DT, PI_UPD_UID, PI_UPD_DT FROM SITE_M_PAGE_INFO  
							WHERE :CR_DT:  = (SELECT MAX(:CR_DT:) FROM SITE_M_PAGE_INFO)";
			$query = $this->db->query($sql,[
	...
	public function update($_id = null){
      ...
      if($_fetch_edit["return_status"] != "0" || $_fetch_edit["return_status"] != "Success" && count($_fetch_edit["result"]) == 0){
      	...
      	foreach ($_exist_data as $key => $value) {
					...
					$_OLD_PI_SYS_ID	=	$value["PI_SYS_ID"];
					$_OLD_PI_ST_SYS_ID	=	$value["PI_ST_SYS_ID"];
					$_OLD_PI_PAGE_DESC	=	$value["PI_PAGE_DESC"];
					$_OLD_PI_CODE	=	$value["PI_CODE"];
					$_OLD_PI_TABLE_1	=	$value["PI_TABLE_1"];
					$_OLD_PI_TABLE_2	=	$value["PI_TABLE_2"];
					$_OLD_PI_TABLE_3	=	$value["PI_TABLE_3"];
					$_OLD_PI_ACTIVE_YN	=	$value["PI_ACTIVE_YN"];
					$_OLD_PI_LANG_CODE	=	$value["PI_LANG_CODE"];
					$_OLD_PI_CR_UID	=	$value["PI_CR_UID"];
					$_OLD_PI_CR_DT	=	$value["PI_CR_DT"];
					$_OLD_PI_UPD_UID	=	$value["PI_UPD_UID"];
					$_OLD_PI_UPD_DT	=	$value["PI_UPD_DT"];
		  	}
		  }
		  
			$_P_PI_SYS_ID = $this->request->getVar("@user_end_parameter_PI_SYS_ID");
			$_P_PI_ST_SYS_ID = $this->request->getVar("@user_end_parameter_PI_ST_SYS_ID");
			$_P_PI_PAGE_DESC = $this->request->getVar("@user_end_parameter_PI_PAGE_DESC");
			$_P_PI_CODE = $this->request->getVar("@user_end_parameter_PI_CODE");
			$_P_PI_TABLE_1 = $this->request->getVar("@user_end_parameter_PI_TABLE_1");
			...
			$params = array(
				...
				array("name"=>":P_PI_SYS_ID", "value"=>$_P_PI_SYS_ID, "type"=>SQLT_CHR, "length"=>22),
				array("name"=>":P_PI_ST_SYS_ID", "value"=>$_P_PI_ST_SYS_ID, "type"=>SQLT_CHR, "length"=>22),
				array("name"=>":P_PI_PAGE_DESC", "value"=>$_P_PI_PAGE_DESC, "type"=>SQLT_CHR, "length"=>250),
				array("name"=>":P_PI_CODE", "value"=>$_P_PI_CODE, "type"=>SQLT_CHR, "length"=>30),
				array("name"=>":P_PI_TABLE_1", "value"=>$_P_PI_TABLE_1, "type"=>SQLT_CHR, "length"=>50),
			...
			$this->db->txn_start_now();
			$datatoShow = $this->db->storedProcedure("SPINE_SITE","UPDATE_SITE_M_PAGE_INFO", $params);
	...
	public function delete($_id = null){
      ...
      if($_fetch_edit["return_status"] != "0" || $_fetch_edit["return_status"] != "Success" && count($_fetch_edit["result"]) == 0){
      	...
      	//$_exist_data = $_fetch_edit["result"];
			...
			$_P_PI_SYS_ID = $this->request->getVar("passing_parameter");
			...
			$this->db->txn_start_now();
			$datatoShow = $this->db->storedProcedure("SPINE_SITE","DELETE_SITE_M_PAGE_INFO", $params);
