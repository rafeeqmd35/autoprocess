  ...
  private function total_count_after_filter($sql=""){
	...
	public function index(){
      ...
      $logged_site_id = $this->request->getVar("logged_site_id");
			...
			$sql = "SELECT * FROM ( SELECT STG_SYS_ID, STG_DESC, STG_ACTIVE_YN, STG_LANG_CODE, STG_CR_UID, STG_CR_DT, STG_UPD_UID, STG_UPD_DT FROM SITE_M_TAG )  ";
			...
			if($result_array){
				...
				foreach ($result_array as $key => $value) {
					$name_array = [
						...
						"@user_end_parameter_STG_SYS_ID"	=>	$value["STG_SYS_ID"],
						"@user_end_parameter_STG_DESC"	=>	$value["STG_DESC"],
						"@user_end_parameter_STG_ACTIVE_YN"	=>	$value["STG_ACTIVE_YN"],
						"@user_end_parameter_STG_LANG_CODE"	=>	$value["STG_LANG_CODE"],
						"@user_end_parameter_STG_CR_UID"	=>	$value["STG_CR_UID"],
						"@user_end_parameter_STG_CR_DT"	=>	$value["STG_CR_DT"],
						"@user_end_parameter_STG_UPD_UID"	=>	$value["STG_UPD_UID"],
						"@user_end_parameter_STG_UPD_DT"	=>	$value["STG_UPD_DT"],
					];
					$data_to_send[] = $name_array;
				}
				$primary_key = "PRIMARY_KEY_NEED_TO_SEND";
        $table_name = "SITE_M_TAG";
        $site_column = "";
	...
	public function create(){
		try{
			...
			$_P_STG_DESC = $this->request->getVar("@user_end_parameter_STG_DESC");
			$_P_STG_ACTIVE_YN = $this->request->getVar("@user_end_parameter_STG_ACTIVE_YN");
			...
			$params = array(
				...
				array("name"=>":P_STG_DESC", "value"=>$_P_STG_DESC, "type"=>SQLT_CHR, "length"=>250),
				array("name"=>":P_STG_ACTIVE_YN", "value"=>$_P_STG_ACTIVE_YN, "type"=>SQLT_CHR, "length"=>1),
			...
			$datatoShow = $this->db->storedProcedure("SPINE_SITE","INSERT_SITE_M_TAG", $params);
	...
	public function new(){
		try{
			...
			$sql = " SELECT STG_SYS_ID, STG_DESC, STG_ACTIVE_YN, STG_LANG_CODE, STG_CR_UID, STG_CR_DT, STG_UPD_UID, STG_UPD_DT FROM SITE_M_TAG  
							WHERE :CR_DT:  = (SELECT MAX(:CR_DT:) FROM SITE_M_TAG)";
			$query = $this->db->query($sql,[
	...
	public function update($_id = null){
      ...
      if($_fetch_edit["return_status"] != "0" || $_fetch_edit["return_status"] != "Success" && count($_fetch_edit["result"]) == 0){
      	...
      	foreach ($_exist_data as $key => $value) {
					...
					$_OLD_STG_SYS_ID	=	$value["STG_SYS_ID"];
					$_OLD_STG_DESC	=	$value["STG_DESC"];
					$_OLD_STG_ACTIVE_YN	=	$value["STG_ACTIVE_YN"];
					$_OLD_STG_LANG_CODE	=	$value["STG_LANG_CODE"];
					$_OLD_STG_CR_UID	=	$value["STG_CR_UID"];
					$_OLD_STG_CR_DT	=	$value["STG_CR_DT"];
					$_OLD_STG_UPD_UID	=	$value["STG_UPD_UID"];
					$_OLD_STG_UPD_DT	=	$value["STG_UPD_DT"];
		  	}
		  }
		  
			$_P_STG_SYS_ID = $this->request->getVar("@user_end_parameter_STG_SYS_ID");
			$_P_STG_DESC = $this->request->getVar("@user_end_parameter_STG_DESC");
			$_P_STG_ACTIVE_YN = $this->request->getVar("@user_end_parameter_STG_ACTIVE_YN");
			$_P_LANG_CODE = $this->request->getVar("@user_end_parameter_LANG_CODE");
			$_P_USER_ID = $this->request->getVar("@user_end_parameter_USER_ID");
			...
			$params = array(
				...
				array("name"=>":P_STG_SYS_ID", "value"=>$_P_STG_SYS_ID, "type"=>SQLT_CHR, "length"=>22),
				array("name"=>":P_STG_DESC", "value"=>$_P_STG_DESC, "type"=>SQLT_CHR, "length"=>250),
				array("name"=>":P_STG_ACTIVE_YN", "value"=>$_P_STG_ACTIVE_YN, "type"=>SQLT_CHR, "length"=>1),
				array("name"=>":P_LANG_CODE", "value"=>$_P_LANG_CODE, "type"=>SQLT_CHR, "length"=>12),
				array("name"=>":P_USER_ID", "value"=>$_P_USER_ID, "type"=>SQLT_CHR, "length"=>32),
			...
			$this->db->txn_start_now();
			$datatoShow = $this->db->storedProcedure("SPINE_SITE","UPDATE_SITE_M_TAG", $params);
	...
	public function delete($_id = null){
      ...
      if($_fetch_edit["return_status"] != "0" || $_fetch_edit["return_status"] != "Success" && count($_fetch_edit["result"]) == 0){
      	...
      	//$_exist_data = $_fetch_edit["result"];
			...
			$_P_STG_SYS_ID = $this->request->getVar("passing_parameter");
			...
			$this->db->txn_start_now();
			$datatoShow = $this->db->storedProcedure("SPINE_SITE","DELETE_SITE_M_TAG", $params);
